/*
    ...
    File: unit.cc
    NTS-3 kaoss pad kit — COLLAGE generic effect unit
    (overlapping micro-loops at multiple playback speeds)

    Preset mapping (X axis, 0-4):
      0 = OFF  — bypass, dry signal passed through
      1 = A    — 1x + 2x (unison + octave up)
      2 = B    — 1x + 0.5x (unison + octave down)
      3 = C    — all 2x (octave up)
      4 = D    — 0.5x + 1x + 2x + 4x (full harmonic stack)

    Activity (depth knob, 0-1023):
      Quantised to nearest 10% to prevent micro-jitter changing the sound.
      Controls number of active loop heads (1-4).
*/

#include "unit_genericfx.h"
#include <climits>
#include "src/params.h"
#include "src/micro_loop.h"
...
public:
  ...
  inline int8_t Init(const unit_runtime_desc_t *desc) {
    ...
    _collage.init(48000.f, bufL, bufR);

    // Initialise at preset A (engine preset 0), 50% activity
    _preset      = 1;       // 1=A in our 0-4 scheme
    _bypassed    = false;
    _activityParam = 511;
    _collage.setPreset(0);                         // engine preset 0 = A
    _collage.setActivity(_quantiseActivity(511));  // 50%
  ...
  inline void Process(const float *in, float *out, uint32_t frames) {
    if (_bypassed) {
      // Preset 0 = OFF: pass dry signal unchanged
      for (uint32_t f = 0; f < frames; ++f) {
        out[f * 2]     = in[f * 2];
        out[f * 2 + 1] = in[f * 2 + 1];
      }
    } else {
      _collage.process(in, out, frames);
    }
  }
  ...
  inline void setParameter(uint8_t id, int32_t value) {
    switch (id) {
      case 0: {
        ...
        _bypassed = (_preset == 0);
        if (!_bypassed)
          _collage.setPreset(_preset - 1);  // map 1-4 → engine 0-3
        break;
      }
      case 1: {
        // ACTIV: quantise to nearest 10% before applying
        _activityParam = clipminmaxi32(0, value, 1023);
        _collage.setActivity(_quantiseActivity(_activityParam));
        break;
      }
      default:
        break;
...
private:
  // Round activity to nearest 10% to prevent micro-jitter from changing sound.
  // 0-1023 → rounded to one of 11 steps (0, 0.1, 0.2 ... 1.0)
  static inline float _quantiseActivity(int32_t raw) {
    const float norm  = (float)raw / 1023.f;        // 0.0 .. 1.0
    const float steps = 10.f;
    const float quantised = (float)(int)(norm * steps + 0.5f) / steps;

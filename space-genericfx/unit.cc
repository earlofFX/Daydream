/*
    ...
    File: unit.cc
    NTS-3 kaoss pad kit — SPACE generic effect unit
    (multi-tap delay with per-tap tonal/pitch manipulation)

    Unlike REARRANGE which varies tap rhythms, SPACE uses fixed linear tap
    spacing and applies a different tonal process to each individual tap.

    Preset mapping (X axis, 0-4):
      0 = OFF — bypass, dry signal passed through
      1 = A   — LPF per tap: earlier taps bright, later taps progressively darker
      2 = B   — resonant BPF per tap: each tap resonates at an overtone frequency
      3 = C   — pitch-shifted taps: alternating fifth up (1.5x) / fourth down (0.75x)
      4 = D   — shimmer: later taps increasingly blend in octave-up (2x) content

    Activity (depth knob, 0-1023):
      Quantised to nearest 10%. Controls number of active taps (1-8).

    Tempo (Y axis, 0-1023 → 40-200 BPM):
      Sets the tap spacing. Overridden by external MIDI clock if connected.

    Always on. Initialises at preset A, 50% activity, 120 BPM.
*/

#include "unit_genericfx.h"
#include <climits>
#include "src/params.h"
#include "src/delay_multi.h"

class Effect {
public:
  Effect() : _preset(1), _bypassed(false), _activityParam(511), _tempoParam(338) {}
  ~Effect() {}
  ...
  inline int8_t Init(const unit_runtime_desc_t *desc) {
    ...
    _space.init(48000.f, bufL, bufR);

    // Initialise at preset A (engine preset 0), 50% activity, 120 BPM
    _preset        = 1;
    _bypassed      = false;
    _activityParam = 511;
    _tempoParam    = 338;
    _space.setPreset(0);
    _space.setActivity(_quantiseActivity(511));
    _applyTempo(338);
  ...
  inline void setParameter(uint8_t id, int32_t value) {
    switch (id) {
      case 0: {
        ...
        _bypassed = (_preset == 0);
        if (!_bypassed)
          _space.setPreset(_preset - 1);  // map 1-4 → engine 0-3
        break;
      }
      case 1: {
        // ACTIV: quantise to nearest 10%
        _activityParam = clipminmaxi32(0, value, 1023);
        _space.setActivity(_quantiseActivity(_activityParam));
        break;
      }
      case 2: {
        // TEMPO: 0-1023 → 40-200 BPM
        _tempoParam = clipminmaxi32(0, value, 1023);
        _applyTempo(_tempoParam);
        break;
      }
      default:
        break;
  ...
  // External MIDI clock overrides manual tempo
  inline void setTempo(uint32_t tempo_fp) {
    _space.setTempo(tempo_fp);
  }
...
private:
  ...
  // Convert 0-1023 param to fixed-point BPM (40-200 BPM range)
  inline void _applyTempo(int32_t paramVal) {
    const float bpm      = 40.f + (float)paramVal / 1023.f * 160.f;
    const uint32_t intP  = (uint32_t)bpm;
    const uint32_t fracP = (uint32_t)((bpm - (float)intP) * 65536.f);
    _space.setTempo((intP << 16) | fracP);
  }

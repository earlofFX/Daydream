/*
    ...
    File: unit.cc
    NTS-3 kaoss pad kit — REARRANGE generic effect unit
    (multi-tap delay with four rhythmic configurations)

    Preset mapping (X axis, 0-4):
      0 = OFF  — bypass, dry signal passed through
      1 = A    — classic linear delay (1, 2, 3, 4 beats)
      2 = B    — syncopated / dotted-8th feel
      3 = C    — triplet grid
      4 = D    — double-time (taps at every half beat)

    Activity (depth knob, 0-1023):
      Quantised to nearest 10% to prevent micro-jitter.
      Controls number of active delay taps (1-8).

    Tempo (Y axis, 0-1023 → 40-200 BPM):
      Overridden by external MIDI clock if connected.
*/
...
public:
  ...
  inline int8_t Init(const unit_runtime_desc_t *desc) {
    ...
    _rearrange.init(48000.f, bufL, bufR);
    ...
    _activityParam = 511;
    _tempoParam    = 338;
    _rearrange.setPreset(0);
    _rearrange.setActivity(_quantiseActivity(511));
    _applyTempo(338);
  ...
  inline void setParameter(uint8_t id, int32_t value) {
    switch (id) {
      case 0: {
        ...
        _bypassed = (_preset == 0);
        if (!_bypassed)
          _rearrange.setPreset(_preset - 1);  // map 1-4 → engine 0-3
        break;
      }
      case 1: {
        // ACTIV: quantise to nearest 10%
        _activityParam = clipminmaxi32(0, value, 1023);
        _rearrange.setActivity(_quantiseActivity(_activityParam));
        break;
      }
      case 2: {
...
private:
  ...
  inline void _applyTempo(int32_t paramVal) {
    ...
    _rearrange.setTempo((intP << 16) | fracP);
  }

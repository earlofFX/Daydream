/*
    ...
    File: unit.cc
    NTS-3 kaoss pad kit — UNDERPASS generic effect unit
    (cyclical micro-loops generating hypnotic drones)

    Preset mapping (X axis, 0-4):
      0 = OFF — bypass, dry signal passed through
      1 = A   — LFO modulates loop length (compresses/lengthens)
      2 = B   — sub-octave drone (0.5x speed) with resonant LPF sweep
      3 = C   — resonant bandpass filter sweep
      4 = D   — envelope-triggered loop length modulation

    Activity (depth knob, 0-1023):
      Quantised to nearest 10%. Controls depth of modifier:
      LFO depth for A/D, filter sweep range for B/C.

    Always on — effect runs continuously, no touch required.
...
public:
  ...
  inline int8_t Init(const unit_runtime_desc_t *desc) {
    ...
    _underpass.init(48000.f, bufL, bufR);

    // Initialise at preset A (engine preset 0), 50% activity
    _preset        = 1;
    _bypassed      = false;
    _activityParam = 511;
    _underpass.setPreset(0);
    _underpass.setActivity(_quantiseActivity(511));
  ...
  inline void Process(const float *in, float *out, uint32_t frames) {
    if (_bypassed) {
      ...
      _underpass.process(in, out, frames);
    }
  }
  ...
  inline void setParameter(uint8_t id, int32_t value) {
    switch (id) {
      case 0: {
        ...
        _bypassed = (_preset == 0);
        if (!_bypassed)
          _underpass.setPreset(_preset - 1);  // map 1-4 → engine 0-3
        break;
      }
      case 1: {
        // ACTIV: quantise to nearest 10%
        _activityParam = clipminmaxi32(0, value, 1023);
        _underpass.setActivity(_quantiseActivity(_activityParam));
        break;
      }
      default:
        break;
